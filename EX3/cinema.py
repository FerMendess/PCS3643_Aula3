...
def cadastrar_valor_ingresso(tipo_sala_param: str, valor_ingresso: int) -> bool:
    sucesso = _tipo_ingresso_ctrl.cadastrar_tipo_ingresso(tipo_sala_param, valor_ingresso)
    if sucesso:
...
def comprarIngressos(  # noqa: N802
    ...
    total = _sessao_ctrl.comprar_ingressos(codigo_sessao, assentos_param, tipos_ingresso)
    if total > 0:
