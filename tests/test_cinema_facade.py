...
class TestCinemaFacade(unittest.TestCase):
    ...
    def test_facade_fluxo_completo(self) -> None:
        ...
        filme_buscado = cinema_facade.buscar_filme(filme.codigo)
        self.assertIsNotNone(filme_buscado)
        assert filme_buscado is not None
        self.assertEqual(filme_buscado.nome, "Matrix")

        cadastrou_preco = cinema_facade.cadastrar_tipo_ingresso("2D", 30)
        ...
        sala_buscada = cinema_facade.buscar_sala(sala.numero)
        ...
        sessao_buscada = cinema_facade.buscar_sessao(sessao.codigo)
        ...
        valor_total = cinema_facade.comprar_ingressos(sessao.codigo, [1, 2], [0, 1])
